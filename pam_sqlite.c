...
static char *format_query(const char *template,
			  struct module_options *options, const char *user,
			  const char *passwd)
{
	char *buf = malloc(256);
	int buflen = 256;
...
static void
get_module_options_from_file(const char *filename,
			     struct module_options *opts, int warn);
...
{
	if (options->database == 0 || options->table == 0
	    || options->user_column == 0) {
		SYSLOG
		    ("the database, table and user_column options are required.");
		return -1;
	}
	return 0;
}
...
static sqlite3 *pam_sqlite_connect(struct module_options *options)
{
	...
	if (rc != SQLITE_OK) {
		SYSLOG("Error opening SQLite database (%s)",
		       sqlite3_errmsg(sdb));
	}
...
static char *encrypt_password(struct module_options *options, const char *pass)
{
	char *s = NULL;
...
#define CRYPT_LEN 13
	...
	free(query);
	if (res != SQLITE_OK) {
		DBGLOG("Error preparing SQLite query (%s)",
		       sqlite3_errmsg(conn));
		return PAM_AUTH_ERR;
	}
...
pam_sm_acct_mgmt(pam_handle_t * pamh, int flags, int argc, const char **argv)
{
	...
	if (options->expired_column || options->sql_check_expired) {
		...
		query =
		    format_query(options->
				 sql_check_expired ? options->sql_check_expired
				 :
	...
	if (options->newtok_column || options->sql_check_newtok) {
		query =
		    format_query(options->
				 sql_check_newtok ? options->sql_check_newtok :
...
pam_sm_chauthtok(pam_handle_t * pamh, int flags, int argc, const char **argv)
{
	...
	if (flags & PAM_PRELIM_CHECK) {
		...
		if ((rc =
		     pam_get_pass(pamh, &pass, PASSWORD_PROMPT,
				  std_flags)) == PAM_SUCCESS) {
			if ((rc =
			     auth_verify_password(user, pass,
						  options)) == PAM_SUCCESS) {
				rc = pam_set_item(pamh, PAM_OLDAUTHTOK,
						  (const void *)pass);
				if (rc != PAM_SUCCESS) {
				...
				DBGLOG("password verification failed for '%s'",
				       user);
				return rc;
			}
		} else {
		...
		if (rc != PAM_SUCCESS) {
			SYSLOG("(%s) user '%s' not authenticated.",
			       pam_get_service(pamh), user);
			free_module_options(options);
			return rc;
		}

		/* get and confirm the new passwords */
		rc = pam_get_confirm_pass(pamh, &newpass, PASSWORD_PROMPT_NEW,
		...
		query =
		    format_query(options->
				 sql_set_passwd ? options->sql_set_passwd :
